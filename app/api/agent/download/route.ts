...
export async function GET(request: Request) {
  ...
  const pairingCode = new URL(request.url).searchParams.get('pairingCode')?.trim().toUpperCase() ?? ''
  if (!/^CD-[A-Z0-9]{3}-[A-Z0-9]{3}$/.test(pairingCode)) return new NextResponse('Invalid pairing code', { status: 400 })

  const device = await db.select().from(remoteDevices).where(and(eq(remoteDevices.userId, session.user.id), eq(remoteDevices.pairingCode, pairingCode)))
  if (!device[0]) return new NextResponse('Pairing code not found', { status: 404 })
...
$body = @{ pairingCode = $pairingCode; identity = @{ deviceId = [guid]::NewGuid().ToString(); hostname = $env:COMPUTERNAME; os = 'windows'; agentVersion = '0.1.0' } } | ConvertTo-Json -Depth 3
$result = Invoke-RestMethod -Uri ($apiUrl.TrimEnd('/') + '/api/agent/enroll') -Method Post -ContentType 'application/json' -Body $body
